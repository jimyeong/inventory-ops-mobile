import { uploadApiClient } from '../../services/ApiService';
import { OptimizedImage } from './ImageOptimisationUtil';
...
class ImageUploadUtil {
  async uploadImage(optimizedImage: OptimizedImage): Promise<ServiceResponse<ImageUploadResult>> {
    try {
      ...
      const response = await uploadApiClient.post('/api/v1/image/upload', formData);
  ...
  async deleteImage(imagePath: string): Promise<ServiceResponse<{ success: boolean; error?: string }> > {
    try {
      const response = await uploadApiClient.delete(`/api/v1/image/delete?imagePath=${imagePath}`);
      return response.data;
    } catch (error: any) {
...
export default new ImageUploadUtil();